...
async function checkForNewChatMessages() {
  if (authStore.isOrganizerRole || !authStore.isAuthenticated) return;
  try {
...
function startChatPolling() {
  if (authStore.isOrganizerRole) return;
  stopChatPolling();
...
const headerTitle = computed(() => (isOrganizerRole.value ? 'Кабинет организатора' : 'Кабинет волонтёра'));

const accountName = computed(() => {
  if (!authStore.user) return 'Гость';
  return authStore.user.full_name || authStore.user.username || (isOrganizerRole.value ? 'Организатор' : 'Волонтёр');
});
...
const profileRoute = computed(() => (isOrganizerRole.value ? '/organizer/profile' : '/volunteer/profile'));
  ...
  <v-app class="protected-layout">
    ...
    >
      ...
      <div class="nav-role-badge">
        <v-icon :icon="isOrganizerRole ? 'mdi-briefcase-outline' : 'mdi-hand-heart-outline'" size="14" />
        {{ isOrganizerRole ? 'Организатор' : 'Волонтёр' }}
      </div>
      ...
      <div class="nav-footer">
        <div class="nav-user">
          ...
          <div class="nav-user__info">
            ...
            <div class="nav-user__role">{{ isOrganizerRole ? 'Организатор' : 'Волонтёр' }}</div>
          </div>
        </div>
      </div>
    </v-navigation-drawer>
