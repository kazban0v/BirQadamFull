...
const isOrganizerApproved = computed(() => authStore.isApprovedOrganizer);
const isOrganizerRole = computed(() => authStore.isOrganizerRole);
const isPendingOrganizer = computed(() => authStore.isPendingOrganizer);
const isRejectedOrganizer = computed(() => authStore.isRejectedOrganizer);
const isVolunteer = computed(() => isAuthenticated.value && !isOrganizerRole.value);
const userName = computed(() => {
  return user.value?.full_name || user.value?.username || '';
});

const dashboardRoute = computed(() => {
  if (isRejectedOrganizer.value) return { name: 'organizer-application-rejected' };
  if (isOrganizerRole.value) return { name: 'organizer-dashboard' };
      ...
      <div class="hero__inner">
        ...
        <p class="hero__p">
          <template v-if="isAuthenticated">
            <template v-if="isOrganizerApproved">
              Управляйте проектами, модереруйте фотоотчёты и общайтесь с командой волонтёров.
            </template>
            <template v-else-if="isPendingOrganizer">
              Заявка организатора на рассмотрении. Зайдите в кабинет — там статус модерации и уведомления.
            </template>
            <template v-else-if="isRejectedOrganizer">
              Заявка на роль организатора отклонена. Откройте кабинет, чтобы увидеть подробности.
            </template>
            <template v-else>
        ...
        <div class="hero__cta">
          ...
          <template v-if="showDashboardLink">
            <RouterLink :to="dashboardRoute" class="cta-btn cta-btn--primary" aria-label="Перейти в кабинет">
              <template v-if="isOrganizerRole">Мой кабинет организатора</template>
