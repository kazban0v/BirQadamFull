...
const contactsForm = ref<VForm | null>(null);
...
const validateCurrentStep = async () => {
  if (step.value === 1) { const { valid } = (await contactsForm.value?.validate()) ?? { valid: false }; return valid; }
  const { valid } = (await accountForm.value?.validate()) ?? { valid: false }; return valid;
};
...
const resetForms = () => {
  [contactsForm, accountForm].forEach(f => { f.value?.reset(); f.value?.resetValidation(); });
  step.value = 1;
};
...
const submit = async () => {
  ...
  try {
    ...
    if (response?.requires_email_verification && response?.user?.email) {
      ...
      showSnackbar('Регистрация завершена. Добро пожаловать!', 'success');
      resetForms();
      setTimeout(() => router.push(response?.dashboard_url || { name: 'volunteer-dashboard' }), 1000);
    }
  } catch (e: any) {
    showSnackbar(e?.response?.data?.detail || 'Не удалось завершить регистрацию.', 'error');
...
const handleVerifyEmail = async () => {
  ...
  try {
    ...
    authStore.persistAccessToken(response);
    await authStore.loadUser();
    showSnackbar('Email подтверждён! Добро пожаловать!', 'success');
    showVerificationDialog.value = false;
    resetForms();
    setTimeout(() => router.push({ name: 'volunteer-dashboard' }), 1000);
  } catch (e: any) {
...
const STEPS = [
  ...
  { n: 2, label: 'Аккаунт',  icon: 'mdi-lock-outline'    },
];
</script>
    ...
    <div class="brand-panel d-none d-lg-flex">
      ...
      <div class="brand-inner">
        <div class="brand-badge">
          <v-icon size="14">mdi-hand-heart</v-icon>
          Волонтёр
        </div>
        ...
        <div class="brand-headline-wrap">
          ...
          <p class="brand-sub">
            Зарегистрируйтесь как волонтёр —<br>
            участвуйте в проектах, берите задачи<br>
            и меняйте мир рядом с вами.
          </p>
        </div>
        ...
        <div class="brand-features">
          <div
            v-for="f in [
              { icon: 'mdi-magnify',                text: 'Находите проекты рядом с вами'  },
              { icon: 'mdi-clipboard-check-outline', text: 'Отслеживайте свои задачи'       },
              { icon: 'mdi-trophy-outline',          text: 'Получайте достижения'           },
              { icon: 'mdi-bell-outline',            text: 'Уведомления о новых акциях'     },
            ]"
            :key="f.text"
            class="brand-feature"
          >
      ...
      <p class="brand-footer-note">
        ...
        Бесплатно · Безопасно · Алматы, Казахстан
      </p>
    </div>
    ...
    <div class="form-panel">
      <div class="form-card">
        ...
        <div class="form-header">
          ...
          <div>
            <h1 class="form-title">Регистрация волонтёра</h1>
        ...
        <div v-show="step === 2" class="step-block">
          <div class="info-notice">
            ...
            <div>
              <strong>Почти готово!</strong>
              <p>Укажите email и пароль — после подтверждения почты вы сможете войти в личный кабинет.</p>
            </div>
          </div>
        ...
        <div class="form-nav">
          ...
          <button v-else class="nav-btn nav-btn--submit" type="button" :disabled="loading" @click="submit">
            ...
            <v-icon v-else size="16">mdi-send-outline</v-icon>
        ...
        <p class="form-hint">
          Хотите создавать проекты?
          <RouterLink to="/register/organizer" class="form-link">Стать организатором</RouterLink>
        </p>
      </div>
    </div>
    ...
    <v-dialog v-model="showVerificationDialog" max-width="400" persistent>
      <v-card class="verify-card pa-6">
        ...
        <div class="d-flex flex-column ga-2">
          ...
          <button class="cancel-link" type="button" @click="handleCancelRegistration">
...
/* ── Левая панель — зелёный как у организатора ──────── */
.brand-panel {
  width: 460px;
  flex-shrink: 0;
...
.verify-title { font-size: 1rem; font-weight: 700; color: #1a1a1a; margin: 0 0 2px; }
.verify-sub   { font-size: 0.78rem; color: #888; margin: 0; }
.verify-hint  { font-size: 0.82rem; color: #888; margin: 0 0 14px; line-height: 1.55; }
...
@media (max-width: 600px) {
  ...
  .stepper-line  { min-width: 20px; margin: 0 6px; }
