# admin_panel/stats_api_views.py
...
import logging
import traceback
from api.models import User, Project, Task, Photo, Activity, VolunteerProject
...
class UserStatsAPIView(APIView):
    ...
    def get(self, request: Request) -> Response:  # type: ignore[override]
        try:
            ...
            logger.error(f'[ERROR] Error in UserStatsAPIView: {e}')
    ...
    def _get_volunteer_stats(self, user: User) -> Dict[str, Any]:  # type: ignore[no-any-unimported]
        ...
        # Достижения
        from api.models import UserAchievement  # type: ignore[attr-defined]
...
class UserActivityStatsAPIView(APIView):
    ...
    def get(self, request: Request) -> Response:  # type: ignore[override]
        try:
            ...
            logger.error(f'[ERROR] Error in UserActivityStatsAPIView: {e}')
            logger.error(traceback.format_exc())
            return Response({
                'success': False,
                'error': str(e),
