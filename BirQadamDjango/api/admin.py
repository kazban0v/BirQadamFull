...
class ProjectAdmin(admin.ModelAdmin):
    list_display = ['id', 'title', 'creator', 'city', 'status', 'volunteers_count', 'active_tasks_count', 'created_at', 'start_date', 'end_date']
    list_filter = ['status', 'city', 'created_at', 'start_date']
    search_fields = ['title', 'description', 'creator__username', 'creator__email', 'city']
    readonly_fields = ['id', 'created_at', 'volunteers_count', 'active_tasks_count']
    date_hierarchy = 'created_at'
    
    def get_queryset(self, request):
        """Добавляем аннотации для счетчиков волонтеров и активных задач"""
        queryset = super().get_queryset(request)
        queryset = queryset.annotate(
            volunteers_count=models.Count('volunteer_projects', filter=models.Q(volunteer_projects__is_active=True), distinct=True),
            active_tasks_count=models.Count('tasks', filter=models.Q(tasks__is_deleted=False, tasks__status__in=['open', 'in_progress', 'under_review']), distinct=True)
        )
        return queryset
    
    def volunteers_count(self, obj):
        """Возвращает количество активных волонтеров в проекте"""
        return obj.volunteers_count
    volunteers_count.short_description = 'Волонтеры'
    volunteers_count.admin_order_field = 'volunteers_count'
    
    def active_tasks_count(self, obj):
        """Возвращает количество активных задач в проекте"""
        return obj.active_tasks_count
    active_tasks_count.short_description = 'Активные задачи'
    active_tasks_count.admin_order_field = 'active_tasks_count'


# Регистрация моделей участия волонтёров в проектах
@admin.register(VolunteerProject)
class VolunteerProjectAdmin(admin.ModelAdmin):
    list_display = ['id', 'volunteer', 'project', 'joined_at', 'is_active']
    list_filter = ['joined_at', 'is_active', 'project__city', 'project__status']
    search_fields = ['volunteer__username', 'volunteer__email', 'project__title', 'project__description']
    readonly_fields = ['id', 'joined_at']
    autocomplete_fields = ['volunteer', 'project']
    
    def get_queryset(self, request):
        queryset = super().get_queryset(request)
        return queryset.select_related('volunteer', 'project')
...
class TaskAdmin(admin.ModelAdmin):
    list_display = ['id', 'text_short', 'project', 'status', 'deadline_date', 'created_at', 'assignments_count', 'completed_count']
    ...
    def get_queryset(self, request):
        """Добавляем аннотации для счетчиков назначений и выполненных заданий"""
        queryset = super().get_queryset(request)
        queryset = queryset.annotate(
            assignments_count=models.Count('taskassignments', distinct=True),
            completed_count=models.Count('taskassignments', filter=models.Q(taskassignments__completed=True), distinct=True)
        )
        return queryset
    
    def assignments_count(self, obj):
        """Общее количество назначений задачи"""
        return obj.assignments_count
    assignments_count.short_description = 'Назначено'
    assignments_count.admin_order_field = 'assignments_count'
    
    def completed_count(self, obj):
        """Количество выполненных назначений"""
        return obj.completed_count
    completed_count.short_description = 'Выполнено'
    completed_count.admin_order_field = 'completed_count'


# Регистрация моделей назначений заданий
@admin.register(TaskAssignment)
class TaskAssignmentAdmin(admin.ModelAdmin):
    list_display = ['id', 'task', 'volunteer', 'accepted', 'completed', 'completed_at']
    list_filter = ['accepted', 'completed', 'completed_at', 'task__project__city']
    search_fields = ['volunteer__username', 'volunteer__email', 'task__text', 'task__project__title']
    readonly_fields = ['id']
    autocomplete_fields = ['volunteer', 'task']
    
    def get_queryset(self, request):
        queryset = super().get_queryset(request)
        return queryset.select_related('volunteer', 'task__project')
...
class ActivityAdmin(admin.ModelAdmin):
    list_display = ['id', 'user', 'type', 'project', 'created_at', 'is_read']
    list_filter = ['type', 'created_at', 'is_read']
    search_fields = ['user__username', 'title', 'description', 'project__title']
    ...
    def has_change_permission(self, request, obj=None):
        return False  # Только для просмотра
...
class UserAchievementAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        queryset = super().get_queryset(request)
        return queryset.select_related('user', 'achievement')
...
class OrganizerApplicationAdmin(admin.ModelAdmin):
    list_display = ['id', 'organization_name', 'user', 'status', 'created_at']
    list_filter = ['status', 'created_at', 'city']
    search_fields = ['user__username', 'user__email', 'organization_name', 'notes']
    readonly_fields = ['id', 'created_at', 'updated_at']
    date_hierarchy = 'created_at'

    fieldsets = (
        ('Основная информация', {
            'fields': ('id', 'user', 'organization_name', 'status')
        }),
        ('Контакты и ссылки', {
            'fields': ('city', 'contact_person', 'website')
        }),
        ('Подробности', {
...
class TrustFactorHistoryAdmin(admin.ModelAdmin):
    list_display = ['id', 'user', 'old_value', 'new_value', 'change_amount', 'reason', 'created_at']
    list_filter = ['reason', 'created_at']
    search_fields = ['user__username', 'reason']
    readonly_fields = ['id', 'user', 'old_value', 'new_value', 'change_amount', 'reason', 
