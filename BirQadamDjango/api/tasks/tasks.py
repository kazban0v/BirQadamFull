...
@shared_task(name='api.tasks.cleanup_old_device_tokens')
def cleanup_old_device_tokens(days: int = 90) -> str:
    ...
    if count > 0:
        ...
        logger.info(f'[OK] Celery: Удалено {deleted_count} старых FCM токенов (>{days} дней)')
        return f'Deleted {deleted_count} tokens'
    else:
        logger.info(f'[OK] Celery: Старых FCM токенов не найдено')
        return 'No old tokens found'


@shared_task(name='api.tasks.send_bulk_notification_task')
def send_bulk_notification_task(notification_id: int) -> str:
    ...
    """
    from admin_panel.services.notification_service import BulkNotificationService  # ✅ Правильный класс
    from api.notifications.models import BulkNotification
    import asyncio
...
@shared_task(name='api.tasks.check_daily_activity')
