...
Сервис уведомлений для тикетов поддержки
"""
...
def send_telegram_notification(ticket, action):
    ...
    """
    try:
        # Telegram уведомления отправляются через admin_panel.services.notification_service
        ...
        if ticket.user.telegram_id:
            ...
            telegram_message = f"📋 <b>Обновление тикета #{ticket.id}</b>\n\n{message}"
            
            async_to_sync(NotificationService.notify_user)(
                ticket.user,
                f"Обновление тикета #{ticket.id}",
                message,
                'ticket_update',
                {'ticket_id': ticket.id, 'action': action},
                telegram_message=telegram_message
            )
            logger.info(f"Telegram уведомление отправлено пользователю {ticket.user.id} о тикете #{ticket.id}")
    except Exception as e:
...
def send_push_notification(ticket, action):
    ...
    """
    try:
        from admin_panel.services.fcm_modern import send_push_notification_to_user
