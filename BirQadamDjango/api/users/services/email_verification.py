...
def generate_verification_code(user: User, email: str) -> str:
    ...
    """
    # Мы больше не деактивируем предыдущие коды, чтобы пользователь мог ввести
    # любой из пришедших ему кодов в течение 15 минут (улучшает UX при задержках почты).
    ...
    for attempt in range(max_attempts):
        ...
        if not VerificationCode.objects.filter(
            code=code,
            email__iexact=email,
            verification_type=VerificationCode.VERIFICATION_REGISTRATION,
            is_used=False,
...
def verify_email_code(email: str, code: str) -> tuple[bool, User | None, str]:
    ...
    try:
        ...
        # Находим любой неиспользованный код для этого email, который совпадает
        ...
        # Убрана строгая проверка на то, что это именно ПОСЛЕДНИЙ сгенерированный код.
        # Если пользователь ввел правильный, неиспользованный и неистекший код, мы его принимаем.
