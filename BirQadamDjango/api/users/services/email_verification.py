...
from api.tasks.tasks import send_verification_email_task
...
"""
    ...
    try:
        # Отправляем задачу в очередь Celery с помощью .delay()
        ...
        success_msg = f"Queued verification email to {email}"
        print(f"[EMAIL] ✓ {success_msg}")
        logger.info(success_msg)
    except Exception as e:
        error_msg = f"Failed to queue email task for {email}: {str(e)}"
