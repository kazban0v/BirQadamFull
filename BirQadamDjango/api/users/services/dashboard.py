...
def get_volunteer_dashboard_data(user) -> Dict[str, Any]:  # type: ignore[no-any-unimported]
    ...
    # exclude(assignments__volunteer=...) может размножить строки Task при JOIN;
    # distinct() гарантирует уникальные задачи в выборке.
    tasks_to_show_qs = all_active_tasks_qs.exclude(assignments__volunteer=user).distinct()
