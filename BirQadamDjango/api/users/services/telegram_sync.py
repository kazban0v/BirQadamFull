"""
Telegram sync service for users
Сервис синхронизации аккаунта веб-портала с Telegram ботом
"""
import secrets
import logging
...
def generate_link_code(user: User) -> str:
    ...
    for attempt in range(max_attempts):
        ...
        # Проверяем, не существует ли уже такой код (только для неиспользованных telegram_link кодов)
    ...
    link_code = VerificationCode.objects.create(
        verification_type='telegram_link',
        code=code,
        user=user,
...
def get_link_code_data(code: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        link_code = VerificationCode.objects.select_related('user').get(
            code=code, 
            verification_type='telegram_link',
            is_used=False
        )
...
def verify_and_link_telegram(code: str, telegram_id: str, telegram_username: str) -> Optional[User]:
    ...
    try:
        with transaction.atomic():
            ...
            link_code = VerificationCode.objects.select_for_update().get(
                id=link_code_id, 
                verification_type='telegram_link',
                is_used=False
            )
...
def get_user_link_code(user: User) -> Optional[str]:
    ...
    """
    try:
        link_code = VerificationCode.objects.filter(
            user=user,
            verification_type='telegram_link',
            is_used=False
        ).exclude(
