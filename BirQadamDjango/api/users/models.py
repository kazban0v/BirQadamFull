...
class OrganizerApplication(models.Model):
    ...
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='organizer_application', verbose_name='Пользователь')
...
class VerificationCode(models.Model):
    ...
    # Значения в БД — часть контракта; не менять строковые литералы без миграции данных.
    VERIFICATION_REGISTRATION = 'email_verification'
    VERIFICATION_PASSWORD_RESET = 'password_reset'

    VERIFICATION_TYPE_CHOICES = (
        ('telegram_link', 'Привязка Telegram'),
        (VERIFICATION_REGISTRATION, 'Подтверждение Email'),
        (VERIFICATION_PASSWORD_RESET, 'Сброс пароля'),
    )
    ...
    def __str__(self) -> str:
        if self.verification_type == 'telegram_link':
            type_display = 'Telegram'
        elif self.verification_type == self.VERIFICATION_PASSWORD_RESET:
            type_display = 'Сброс пароля'
        else:
            type_display = 'Email'
    ...
    def clean(self) -> None:
        ...
        if self.verification_type in (
            self.VERIFICATION_REGISTRATION,
            self.VERIFICATION_PASSWORD_RESET,
        ) and not self.email:
            raise ValidationError({'email': 'Email обязателен для этого типа верификации'})
