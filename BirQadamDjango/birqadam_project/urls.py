...
def frontend_view(request, path=''):
    ...
    frontend_dist_path = settings.BASE_DIR / 'portal_frontend' / 'dist'
    ...
    if index_path.exists():
        ...
        return HttpResponse('Frontend not found. Please build the frontend first: cd portal_frontend && npm run build', status=404)
...
def serve_frontend_file(request, path, document_root=None):
    ...
    if document_root is None:
        document_root = str(settings.BASE_DIR / 'portal_frontend' / 'dist')
...
def serve_assets(request, path):
    """Обслуживание файлов из portal_frontend/dist/assets"""
    return serve_frontend_file(request, path, document_root=str(settings.BASE_DIR / 'portal_frontend' / 'dist' / 'assets'))

def serve_frontend_root(request, path):
    """Обслуживание файлов из корня portal_frontend/dist (birqadam-logo.png, vite.svg и т.д.)"""
    return serve_frontend_file(request, path, document_root=str(settings.BASE_DIR / 'portal_frontend' / 'dist'))
...
urlpatterns = [
    ...
    path('api/web/', include(('api.api.web_portal', 'web_portal'), namespace='web_portal')),
