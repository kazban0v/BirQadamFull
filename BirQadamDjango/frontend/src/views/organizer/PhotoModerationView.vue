...
async function openPhotoDetail(photoId: number) {
  ...
  try {
    const detail = await organizerStore.ensurePhotoDetail(photoId);
    console.log('📸 Photo detail loaded:', {
      photoId,
      hasPhotos: detail?.photos?.length > 0,
      photosCount: detail?.photos?.length || 0,
      hasImageUrl: !!detail?.image_url,
...
<template>
  <div class="photos-view">
                    ...
                    <v-card class="photo-card" variant="text" border rounded="lg">
                      ...
                      <div class="photo-card__actions">
                        <v-btn
                          variant="text"
                          color="primary"
                          class="text-none font-weight-semibold photo-card__action-view"
                          size="small"
                          @click="openPhotoDetail(photo.id)"
                        >
                          <v-icon icon="mdi-eye-outline" start size="18" />
                          Смотреть
                        </v-btn>
                        <div class="photo-card__actions-buttons">
                          <template v-if="photo.status === 'pending'">
                            <v-btn
                              color="success"
                              class="text-none photo-card__action-btn"
                              variant="flat"
                              size="small"
                              :loading="photoActionLoading(photo.id)"
                              @click="openApproveDialog(photo.id)"
                            >
                              <v-icon icon="mdi-check-circle" start size="18" />
                              <span class="photo-card__action-text">Одобрить</span>
                            </v-btn>
                            <v-btn
                              color="error"
                              class="text-none photo-card__action-btn photo-card__action-reject"
                              variant="flat"
                              size="small"
                              :loading="photoActionLoading(photo.id)"
    ...
    <v-dialog v-model="detailDialog.open" max-width="720" :fullscreen="mobile" class="detail-dialog-mobile">
      <v-card class="detail-dialog" rounded="xl">
        <v-card-title class="d-flex align-center justify-space-between detail-dialog__title">
          <div class="detail-dialog__title-content">
        ...
        <v-card-text class="detail-dialog__text">
          <v-skeleton-loader
          ...
          <div v-else-if="currentDetail" class="detail-dialog__content">
            <div class="detail-dialog__image-container">
              <!-- Используем photos если есть, иначе fallback на image_url -->
              ...
              <!-- Fallback на основное изображение если photos пустой -->
              <template v-else>
                <v-img
              ...
              <div class="detail-dialog__navigation" v-if="currentDetail.photos && currentDetail.photos.length > 1">
                <v-btn
                  icon="mdi-chevron-left"
                  variant="text"
                  color="primary"
                  :disabled="detailDialog.slide === 0"
                  @click="detailDialog.slide = Math.max(0, detailDialog.slide - 1)"
                  class="detail-dialog__nav-btn"
                />
                <span class="detail-dialog__slide-counter">
                  {{ detailDialog.slide + 1 }} / {{ currentDetail.photos.length }}
                </span>
                <v-btn
                  icon="mdi-chevron-right"
                  variant="text"
                  color="primary"
                  :disabled="detailDialog.slide === currentDetail.photos.length - 1"
                  @click="detailDialog.slide = Math.min(currentDetail.photos.length - 1, detailDialog.slide + 1)"
                  class="detail-dialog__nav-btn"
                />
              </div>
            </div>

            <div class="detail-dialog__thumbnails" v-if="currentDetail.photos && currentDetail.photos.length > 1">
              <v-avatar
                v-for="(item, index) in currentDetail.photos"
                :key="`thumb-${item.id}`"
...
.detail-dialog__image-container {
  position: relative;
  width: 100%;
}
...
.detail-dialog__nav-btn {
  flex-shrink: 0;
}

.detail-dialog__slide-counter {
  font-size: 0.875rem;
  font-weight: 600;
...
.detail-dialog__thumbnail:hover {
...
.detail-dialog__meta-row > span:first-child {
  font-weight: 500;
  min-width: 100px;
}
...
@media (max-width: 599px) {
  .detail-dialog-mobile .v-dialog {
    margin: 0;
  }
  
  .detail-dialog-mobile .v-card {
    border-radius: 0;
    height: 100vh;
    display: flex;
    flex-direction: column;
  ...
  .detail-dialog-mobile .v-card-text {
    flex: 1;
    overflow-y: auto;
    padding: 16px;
  }
