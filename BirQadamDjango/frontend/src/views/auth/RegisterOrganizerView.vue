...
const rules = {
  ...
  url: (value: string) => {
    ...
    try {
      // Разрешаем ввод без протокола - просто проверяем, что это похоже на URL
      // Можно вводить: example.com, example.kz, https://example.com и т.д.
      const trimmed = value.trim();
      
      // Если есть протокол - проверяем как полный URL
      if (/^https?:\/\//i.test(trimmed)) {
        new URL(trimmed);
        return true;
      }
      
      // Если нет протокола - проверяем базовую структуру (домен)
      // Разрешаем: example.com, subdomain.example.kz, example.kz/path и т.д.
      const urlPattern = /^([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}(\/.*)?$/i;
      if (urlPattern.test(trimmed)) {
        return true;
      }
      
      return 'Введите корректный URL (например: example.kz или https://example.kz)';
    } catch (error) {
      return 'Введите корректный URL (например: example.kz или https://example.kz)';
    }
  },
...
const submit = async () => {
  ...
  try {
    ...
    // Обрабатываем website: добавляем протокол если его нет
    let websiteValue = formState.website?.trim();
