...
class RateLimitMiddleware:
    ...
    def is_rate_limited(self, ip_address, path):
        ...
        # Увеличиваем счётчик безопасно (locmem cache может бросать ValueError при гонках/очистке ключа)
        if current_requests == 0:
            # Первый запрос - устанавливаем TTL
            cache.set(cache_key, 1, period)
        else:
            try:
                # Увеличиваем счётчик без изменения TTL
                cache.incr(cache_key)
            except ValueError:
                # Ключ мог исчезнуть между get() и incr() — восстанавливаем
                cache.set(cache_key, 1, period)
