...
import sys
# Создаем путь к логам относительно корня проекта
...
from telegram_bot.telegram_bot import application
...
def get_user(telegram_id: str) -> Any:
    from api.models import User  # Lazy import
    try:
...
def link_telegram_account(code: str, telegram_id: str, telegram_username: str) -> Any:
    ...
    from api.users.services.telegram_sync import verify_and_link_telegram
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    from telegram_bot.organization_handlers import org_menu  # Lazy import
    from telegram_bot.volunteer_handlers import volunteer_menu as volunteer_start  # Lazy import
...
async def receive_phone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    from api.utils.utils import normalize_phone  # Lazy import
...
async def receive_role(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if role == "role_volunteer":
        from telegram_bot.volunteer_handlers import get_volunteer_keyboard  # Lazy import
...
async def receive_organization(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
                from telegram_bot.volunteer_handlers import get_volunteer_keyboard  # Lazy import (используется для новых организаторов)
...
async def link_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if len(parts) > 1:
        ...
        if code.isdigit() and len(code) == 6:
            ...
            if user:
                # Успешная привязка
                from telegram_bot.volunteer_handlers import get_volunteer_keyboard
                from telegram_bot.organization_handlers import org_menu
                ...
                if user.is_organizer and user.is_approved:
                    ...
                    from telegram_bot.volunteer_handlers import volunteer_menu
...
if __name__ == '__main__':
    ...
    from telegram_bot.volunteer_handlers import register_handlers as register_volunteer_handlers
    from telegram_bot.organization_handlers import register_handlers as register_organization_handlers
