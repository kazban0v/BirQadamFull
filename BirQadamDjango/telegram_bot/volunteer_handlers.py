...
from api.models import User, Project, Photo, VolunteerProject, Task, TaskAssignment
...
def get_volunteer_project(volunteer: User) -> tuple[Optional[VolunteerProject], Optional[str]]:
    ...
    birqadam_project = VolunteerProject.objects.filter(volunteer=volunteer).select_related('project').first()
    if birqadam_project:
        logger.info(f"Volunteer project found: {birqadam_project.project.title}")
        return birqadam_project, birqadam_project.project.title
...
def create_volunteer_project(volunteer: User, project: Project) -> tuple[Optional[VolunteerProject], Optional[str]]:
    ...
    try:
        with transaction.atomic():
            ...
            birqadam_project = VolunteerProject.objects.create(volunteer=volunteer, project=project)  # type: ignore[attr-defined]
            logger.info(f"Volunteer project created: {birqadam_project.id}")  # type: ignore[attr-defined]
        return birqadam_project, project.title
...
@sync_to_async
def delete_volunteer_project(birqadam_project: VolunteerProject) -> None:
    logger.info(f"Deleting volunteer project {birqadam_project.id}")  # type: ignore[attr-defined]
    birqadam_project.delete()
    logger.info(f"Volunteer project {birqadam_project.id} deleted")  # type: ignore[attr-defined]
...
async def handle_join_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if 0 <= choice < len(projects):
        ...
        birqadam_project, project_title = await create_volunteer_project(db_user, project)
        if birqadam_project:
...
async def handle_leave_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if 0 <= choice < len(projects):
        birqadam_project = projects[choice][0]
        project_title = projects[choice][1]
        await delete_volunteer_project(birqadam_project)
        if query.message:
...
async def task_photo_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message and update.message.photo:
        ...
        try:
            ...
            from telegram_bot.telegram_feedback_helpers import (
