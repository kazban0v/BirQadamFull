...
const getStatusConfig = (status?: PhotoReport['status']) => {
  switch (status) {
    case 'approved':
      return {
        label: 'Одобрено',
...
const getPointsWord = (value: number) => {
...
const getFallbackFeedback = (report: PhotoReport) => {
  if (report.status === 'approved') {
    return 'Организатор проверил отчет и подтвердил выполнение задачи.';
  }

  if (report.status === 'rejected') {
    return report.rejection_reason || 'Отчет возвращен на доработку.';
  }

  return 'Организатор еще не оставил комментарий. Отчет находится на проверке.';
};
...
export const PhotoReportDetailScreen: React.FC = () => {
  ...
  const volunteerName = user?.full_name || user?.username || 'Волонтер';
  ...
  const feedbackSource = task?.creator_name || latestReport?.project_title || 'Организатор проекта';
  const ratingValue = Math.max(0, Math.min(5, latestReport?.rating ?? 0));
  const pointsValue = latestReport?.rating || task?.reward_points || 0;
  const submittedLabel = formatDate(latestReport?.uploaded_at || task?.photo_uploaded_at);
  ...
  const loadReportDetails = useCallback(async () => {
    ...
    try {
      ...
      if (!reportsData.length) {
        Alert.alert('Отчет не найден', 'По этой задаче пока нет доступного фотоотчета.', [
          {
            text: 'Назад',
      ...
      const errorMessage =
        ...
        'Не удалось загрузить подробную информацию об отчете.';
  ...
  const handleShare = useCallback(async () => {
    ...
    const statusLabel = getStatusConfig(latestReport.status).label;
    const shareMessage = [
      'Подробная информация об отчете',
      task?.title ? `Задача: ${task.title}` : null,
      `Статус: ${statusLabel}`,
      `Дата отправки: ${submittedLabel}`,
      latestReport.volunteer_comment ? `Комментарий волонтера: ${latestReport.volunteer_comment}` : null,
    ]
      .filter(Boolean)
      .join('\n');
    ...
    try {
      await Share.share({
        title: 'Подробная информация об отчете',
        message: shareMessage,
      });
    } catch {
      Alert.alert('Ошибка', 'Не удалось открыть меню отправки.');
    }
  }, [latestReport, submittedLabel, task?.title]);
  ...
  if (!latestReport) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.emptyTitle}>Отчет пока недоступен</Text>
      </View>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, isCompact && styles.headerTitleCompact]}>Подробности отчета</Text>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        <View style={[styles.heroContainer, isNarrow && styles.heroContainerNarrow]}>
          ...
          {pointsValue > 0 && latestReport.status === 'approved' && (
            <View style={[styles.pointsBadge, isCompact && styles.pointsBadgeCompact]}>
              ...
              <Text style={[styles.pointsText, isCompact && styles.pointsTextCompact]}>{`+${pointsValue} ${getPointsWord(pointsValue)}`}</Text>
            </View>
          )}
        </View>
        ...
        <View style={[styles.contentCard, isCompact && styles.contentCardCompact, isNarrow && styles.contentCardNarrow]}>
          <View style={[styles.userRow, isCompact && styles.userRowCompact]}>
            <View style={[styles.userInfoRow, isCompact && styles.userInfoRowCompact]}>
              ...
              <View style={styles.userMeta}>
                ...
                <Text style={styles.userDate}>Отправлено {submittedLabel}</Text>
              </View>
            </View>
          ...
          <Text style={styles.sectionLabel}>КОММЕНТАРИЙ ВОЛОНТЕРА</Text>
          <Text style={[styles.sectionText, isCompact && styles.sectionTextCompact]}>
            {latestReport.volunteer_comment?.trim() || 'Комментарий волонтера не добавлен.'}
          </Text>
          ...
          <View style={[styles.feedbackCard, isCompact && styles.feedbackCardCompact]}>
            <View style={[styles.feedbackHeader, isCompact && styles.feedbackHeaderCompact]}>
              <Text style={[styles.feedbackTitle, isCompact && styles.feedbackTitleCompact]}>Отзыв организатора</Text>
            ...
            <Text style={[styles.feedbackText, isCompact && styles.feedbackTextCompact]}>
              {latestReport.organizer_comment?.trim() || getFallbackFeedback(latestReport)}
            </Text>
          </View>
        </View>
      </ScrollView>
