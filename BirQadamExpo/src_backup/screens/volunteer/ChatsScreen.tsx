...
const ChatRow = ({
  ...
  const messageText = item.last_message?.text || 'Сообщений пока нет';
  ...
  const renderEmptyState = () => {
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyTitle}>{isFiltered ? 'Ничего не найдено' : 'Пока нет чатов'}</Text>
        <Text style={styles.emptySubtitle}>
          {isFiltered
            ? 'Попробуйте изменить поиск или снять фильтры.'
            : 'Чаты появляются только по активным проектам. Присоединяйтесь к проекту, чтобы начать общение.'}
        </Text>
        <TouchableOpacity
          style={styles.exploreButton}
          onPress={() => {
            if (isFiltered) { setSearchQuery(''); setShowUnreadOnly(false); setActiveFilter('all'); return; }
            navigation.navigate('VolunteerProjects');
          }}
        >
          <Text style={styles.exploreButtonText}>{isFiltered ? 'Сбросить фильтры' : 'Найти проекты'}</Text>
        </TouchableOpacity>
      </View>
    );
  };
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.chipsRow}>
        {(['all', 'project',] as const).map((f) => (
          <FilterChip
            key={f}
            label={{ all: 'Все', project: 'Проекты', }[f]}
            ...
            <Pressable onPress={() => undefined}>
              {[
                ...
                },
                {
                  ...
                  label: showUnreadOnly ? 'Показать все чаты' : 'Только непрочитанные',
