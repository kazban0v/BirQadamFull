...
export const PermissionCard: React.FC<PermissionCardProps> = ({
  ...
  const getCardStyle = () => {
    switch (type) {
      case 'notifications':
        return {
          gradient: ['#FF6B6B' as const, '#EE5A5A' as const],
      ...
      case 'location':
        return {
          gradient: ['#4F46E5' as const, '#6366F1' as const],
      ...
      case 'network':
        return {
          gradient: ['#F59E0B' as const, '#FBBF24' as const],
      ...
      default:
        return {
          gradient: ['#6B7280' as const, '#9CA3AF' as const],
    ...
    return (
      <View style={[styles.card, styles.cardGranted]}>
        <View style={styles.cardContent}>
          <View style={[styles.iconContainerGranted, { backgroundColor: '#ECFDF5' }]}>
            <Ionicons name="checkmark-circle" size={36} color="#10B981" />
          </View>
          <View style={styles.textContainer}>
            <Text style={styles.titleGranted}>{title}</Text>
            <Text style={styles.descriptionGranted}>Разрешение предоставлено</Text>
          </View>
        </View>
      </View>
    );
