...
const formatEventTime = (event: CalendarEvent, t: (key: string) => string) => {
  if (event.is_all_day) {
...
const getEventLocation = (event: CalendarEvent, t: (key: string) => string) =>
...
const getEventNodeTitle = (event: CalendarEvent, t: (key: string) => string) => {
  if (event.source_type === 'project') {
    return getEventTypeConfig(event.type, t).label;
  }
...
export const VolunteerCalendarScreen: React.FC = () => {
  ...
  const groupedSelectedEvents = useMemo(() => {
    const groups = selectedEvents.reduce<Map<string, CalendarEventGroup>>((accumulator, event) => {
      ...
      accumulator.set(key, {
        ...
        location: getEventLocation(event, t),
  ...
  const legendItems = useMemo(
    () => ['project_start', 'project_end', 'task_deadline'].map((type) => getEventTypeConfig(type, t)),
    [t]
  );
      ...
      >
          ...
          <View style={styles.daysGrid}>
            {monthDays.map((day) => {
              ...
              const dayDots = Array.from(
                new Set(dayEvents.map((event) => getEventTypeConfig(event.type, t).color))
              ).slice(0, 4);
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>
            {language === 'kk'
              ? `${formatSelectedDateTitle(selectedDateKey)}${t('calendar.s_25')}`
              : `${t('calendar.s_25')}${formatSelectedDateTitle(selectedDateKey)}`}
          </Text>
              ...
              <View key={group.key} style={styles.projectTreeCard}>
                    ...
                    <View style={styles.projectTreeTagsRow}>
                      {projectEventCount ? (
                        <View style={styles.projectTreeTag}>
                          ...
                          <Text style={styles.projectTreeTagText}>{projectEventCount} {t('calendar.s_26')}</Text>
                        </View>
                      ) : null}
                      ...
                      {taskEventCount ? (
                        <View style={styles.projectTreeTag}>
                          ...
                          <Text style={styles.projectTreeTagText}>{taskEventCount} {t('calendar.s_27')}</Text>
                        </View>
                      ) : null}
                    </View>
                  </View>
                ...
                <View style={styles.projectTreeChildren}>
                  {groupEvents.map((event, index) => {
                    const typeConfig = getEventTypeConfig(event.type, t);
                      ...
                      >
                        ...
                        <View style={styles.treeEventBody}>
                          ...
                          <View style={styles.eventMetaRow}>
                            ...
                            <Text style={styles.eventMetaText}>{formatEventTime(event, t)}</Text>
                          </View>

                          <View style={styles.eventMetaRow}>
                            <Ionicons name="location-outline" size={14} color={appColors.textMuted} />
                            <Text style={styles.eventMetaText} numberOfLines={1}>
                              {getEventLocation(event, t)}
                            </Text>
                          </View>
                        </View>
                      </TouchableOpacity>
