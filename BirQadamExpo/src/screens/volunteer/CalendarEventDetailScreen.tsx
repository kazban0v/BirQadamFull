...
const getEventTypeConfig = (type: string, t: (key: string) => string) => {
  switch (type) {
    case 'project_start':
      return {
        label: t('calendareventdetail.s_0'),
    ...
    case 'project_end':
      return {
        label: t('calendareventdetail.s_1'),
    ...
    default:
      return {
        label: t('calendareventdetail.s_2'),
...
const getStatusConfig = (status: string | null | undefined, t: (key: string) => string) => {
  switch (status) {
    case 'completed':
      return {
        label: t('calendareventdetail.s_3'),
        color: '#15803D',
        backgroundColor: appColors.primarySurfaceStrong,
      };
    case 'approved':
      return {
        label: t('calendareventdetail.s_4'),
        color: '#047857',
        backgroundColor: appColors.primarySurfaceStrong,
      };
    case 'under_review':
      return {
        label: t('calendareventdetail.s_5'),
        color: '#047857',
        backgroundColor: appColors.primarySurfaceStrong,
      };
    case 'revision':
      return {
        label: t('calendareventdetail.s_6'),
        color: '#2563EB',
        backgroundColor: '#DBEAFE',
      };
    case 'rejected':
      return {
        label: t('calendareventdetail.s_7'),
        color: '#BE123C',
        backgroundColor: '#FFE4E6',
      };
    case 'in_progress':
      return {
        label: t('calendareventdetail.s_8'),
        color: appColors.primary,
        backgroundColor: appColors.surfaceMuted,
      };
    case 'archived':
      return {
        label: t('calendareventdetail.s_9'),
        color: appColors.textMuted,
        backgroundColor: appColors.surfaceMuted,
      };
    case 'open':
      return {
        label: t('calendareventdetail.s_10'),
        color: '#0F766E',
...
const formatLongDate = (value: string, localeTag: string) => {
...
const formatTimeRange = (
  ...
  if (event.is_all_day) {
    return t('calendareventdetail.s_11');
  }
  ...
  if (start && end) {
    return t('calendareventdetail.s_37', { start, end });
  }
  ...
  return t('calendareventdetail.s_12');
};

const getLocation = (event: CalendarEvent, t: (key: string, vars?: Record<string, string | number>) => string) =>
  event.location || event.project_address || event.project_city || t('calendareventdetail.s_13');
...
const buildShareMessage = (
  ...
  return [
    event.title,
    event.subtitle || null,
    `${t('calendareventdetail.s_21')}: ${formatLongDate(event.date, localeTag)}`,
    `${t('calendareventdetail.s_22')}: ${formatTimeRange(event, t)}`,
    `${t('calendareventdetail.s_23')}: ${getLocation(event, t)}`,
...
export const CalendarEventDetailScreen: React.FC = () => {
  ...
  const typeConfig = useMemo(() => getEventTypeConfig(event.type, t), [event.type, t]);
  const statusConfig = useMemo(() => getStatusConfig(event.status, t), [event.status, t]);
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.screen} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>{t('calendareventdetail.s_20')}</Text>
        ...
        <View style={styles.metaCard}>
          <View style={styles.metaRow}>
            ...
            <View style={styles.metaTextBlock}>
              <Text style={styles.metaLabel}>{t('calendareventdetail.s_21')}</Text>
              <Text style={styles.metaValue}>{formatLongDate(event.date, localeTag)}</Text>
            </View>
          </View>
          ...
          <View style={styles.metaRow}>
            ...
            <View style={styles.metaTextBlock}>
              <Text style={styles.metaLabel}>{t('calendareventdetail.s_22')}</Text>
              <Text style={styles.metaValue}>{formatTimeRange(event, t)}</Text>
            </View>
          </View>
          ...
          <View style={styles.metaRow}>
            ...
            <View style={[styles.metaTextBlock, styles.metaTextBlockWide]}>
              <Text style={styles.metaLabel}>{t('calendareventdetail.s_23')}</Text>
              <Text style={styles.metaValue}>{getLocation(event, t)}</Text>
            </View>

            {canShowMap ? (
              <TouchableOpacity style={styles.mapButton} onPress={handleOpenMap} activeOpacity={0.9}>
                <Text style={styles.mapButtonText}>{t('calendareventdetail.s_24')}</Text>
              </TouchableOpacity>
            ) : null}
          </View>
        </View>
        ...
        {statusConfig ? (
          <View style={styles.statusRow}>
            <Text style={styles.sectionTitle}>{t('calendareventdetail.s_25')}</Text>
        ...
        <View style={styles.sectionCard}>
          <Text style={styles.sectionTitle}>{t('calendareventdetail.s_26')}</Text>
          <Text style={styles.descriptionText}>
            {event.description || t('calendareventdetail.s_27')}
          </Text>
        </View>

        <View style={styles.sectionCard}>
          <Text style={styles.sectionTitle}>{t('calendareventdetail.s_28')}</Text>
          ...
          {participantsPreview.length ? (
            <View style={styles.participantsRow}>
              ...
              <Text style={styles.participantsText}>{event.participants_count} {t('calendareventdetail.s_29')}</Text>
            </View>
          ) : (
            <Text style={styles.placeholderText}>{t('calendareventdetail.s_30')}</Text>
          )}
        </View>
          ...
          <View style={styles.sectionCard}>
            <View style={styles.reminderRow}>
              <View style={styles.reminderTextBlock}>
                <Text style={styles.sectionTitle}>{t('calendareventdetail.s_31')}</Text>
                <Text style={styles.placeholderText}>
                  {reminderEnabled
                    ? t('calendareventdetail.s_32')
                    : t('calendareventdetail.s_33')}
                </Text>
              </View>
        ...
        {event.project_id ? (
          <TouchableOpacity style={styles.relatedCard} onPress={openProject} activeOpacity={0.92}>
            ...
            <View style={styles.relatedTextBlock}>
              <Text style={styles.relatedLabel}>{t('calendareventdetail.s_34')}</Text>
              <Text style={styles.relatedTitle}>{event.project_title || t('calendareventdetail.s_35')}</Text>
        ...
        {event.task_id ? (
          <TouchableOpacity style={styles.secondaryButton} onPress={openTask} activeOpacity={0.9}>
            ...
            <Text style={styles.secondaryButtonText}>{t('calendareventdetail.s_36')}</Text>
          </TouchableOpacity>
        ) : null}
      </ScrollView>
