...
export const SubmitPhotoReportScreen: React.FC = () => {
  ...
  const loadTask = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        ...
        'Не удалось загрузить задачу для фотоотчета.';
  ...
  useEffect(() => {
    if (!loading && task && !canUploadForTask) {
      Alert.alert('Фотоотчет недоступен', 'Сейчас для этой задачи нельзя отправить фотоотчет.', [
        {
          text: 'Назад',
          onPress: () => navigation.goBack(),
        },
    ...
    setSelectedPhotos((current) => {
      ...
      for (const asset of assets) {
        ...
        if (asset.fileSize && asset.fileSize > MAX_FILE_SIZE_BYTES) {
          Alert.alert('Файл слишком большой', 'Один из файлов превышает 10 МБ и не был добавлен.');
          continue;
        }
  ...
  const handlePickFromLibrary = useCallback(async () => {
    if (selectedPhotos.length >= MAX_PHOTOS) {
      Alert.alert('Лимит фотографий', `Можно прикрепить максимум ${MAX_PHOTOS} фотографий.`);
      return;
    }

    const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permission.granted) {
      Alert.alert('Нет доступа', 'Разрешите доступ к галерее, чтобы выбрать фотографии.');
      return;
    }
  ...
  const handleTakePhoto = useCallback(async () => {
    ...
    if (!permission.granted) {
      Alert.alert('Нет доступа', 'Разрешите доступ к камере, чтобы сделать фотографию.');
      return;
    }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!selectedPhotos.length) {
      Alert.alert('Добавьте фото', 'Нужно выбрать хотя бы одну фотографию для фотоотчета.');
      return;
    }

    if (isTaskExpired(task)) {
      Alert.alert('Срок истёк', 'Для просроченной задачи фотоотчёт загрузить нельзя.');
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        'Успешно',
        task.status === 'revision'
          ? 'Фотоотчет отправлен повторно и передан на проверку.'
          : 'Фотоотчет отправлен и передан на проверку.',
        [
          {
            text: 'OK',
            onPress: () => navigation.goBack(),
      ...
      const errorMessage =
        error?.response?.data?.detail ||
        error?.response?.data?.error ||
        'Не удалось отправить фотоотчет.';
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Фотоотчет</Text>
      ...
      >
        <View style={styles.card}>
          <Text style={styles.cardLabel}>ЗАДАЧА</Text>
          <Text style={styles.taskTitle}>{task.title || task.description || 'Без названия'}</Text>
          <Text style={styles.taskSubtitle}>
            После отправки фотоотчет перейдет в статус «На проверке».
          </Text>
        </View>
        ...
        {task.status === 'revision' && (
          <View style={[styles.card, styles.warningCard]}>
            <View style={styles.warningHeader}>
              ...
              <Text style={styles.warningTitle}>Нужно доработать фотоотчет</Text>
            </View>
            <Text style={styles.warningText}>
              {task.rejection_reason || 'Организатор попросил отправить фотоотчет повторно.'}
            </Text>
          </View>
        )}
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.sectionHint}>Можно прикрепить до 5 фотографий.</Text>
          ...
          <View style={styles.actionsRow}>
            <TouchableOpacity style={styles.secondaryAction} onPress={handlePickFromLibrary}>
              ...
              <Text style={styles.secondaryActionText}>Из галереи</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity style={styles.secondaryAction} onPress={handleTakePhoto}>
              ...
              <Text style={styles.secondaryActionText}>Сделать фото</Text>
            </TouchableOpacity>
          </View>
          ...
          {selectedPhotos.length ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>Фотографии еще не выбраны</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.card}>
          ...
          {comment.trim().length ? (
            ...
            >
              ...
              <Text style={styles.commentClearActionText}>Очистить</Text>
            </TouchableOpacity>
          ) : null}
          ...
          <TextInput
            ...
            placeholder="При необходимости добавьте короткое пояснение"
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
        <Text style={styles.footerHint}>{selectedPhotos.length} / {MAX_PHOTOS} фото выбрано</Text>
        <TouchableOpacity
          ...
          {submitting ? (
            ...
            <>
              ...
              <Text style={styles.submitButtonText}>
                {task.status === 'revision' ? 'Отправить повторно' : 'Отправить фотоотчет'}
              </Text>
            </>
          )}
        </TouchableOpacity>
