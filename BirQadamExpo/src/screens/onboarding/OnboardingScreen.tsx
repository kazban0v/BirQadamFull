...
export const OnboardingScreen: React.FC<OnboardingScreenProps> = ({ onComplete }) => {
  ...
  const renderPage = (page: OnboardingPage, index: number) => {
    ...
    return (
      <View key={page.id} style={[styles.page, { paddingTop: insets.top + 16 }]}>
        {/* Шапка (Header) статичная */}
        <View style={styles.header}>
          <View style={styles.logo}>
            <Ionicons name="leaf" size={32} color={appColors.primary} />
        ...
        <View style={styles.pagination}>
          {onboardingPages.map((_, idx) => {
            ...
            const dotColor = scrollX.interpolate({
              inputRange: dotInputRange,
              outputRange: ['#D1D5DB', appColors.primary, '#D1D5DB'],
              extrapolate: 'clamp',
            });
